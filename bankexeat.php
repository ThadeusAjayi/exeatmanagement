...
if(!isset($_SESSION['user'])){
	...
	if(isset($_POST['submit'])){
		header("Location: bankexeat.php?status=error&msg=Leave application submitted successfully");
			if($_POST['hall'] != "" && $_POST['purpose'] != "" && $_POST['destination'] != "" && $_POST['sleave'] != "" && $_POST['sreturn'] != ""){
				$type = "Bank Exeat";
				...
				//As time
				$startDate = $_POST['sleave'];
				$endDate = $_POST['sreturn'];
				...
				$sql = "INSERT INTO leave_application (leave_type,leave_name,leave_matric,leave_course,leave_level,leave_hall,leave_purpose,leave_destination,leave_parent,leave_pcontact,leave_sdate,leave_edate) VALUES ('$type','$name','$matric','$course','$level','$hall','$purpose','$destination','$parent','$pcontact','$startDate','$endDate')";	
					if(mysqli_query($con, $sql)){
						header("Location: bankexeat.php?status=error&msg=Leave application submitted successfully") or die(mysqli_error($con, $sql));
					}
			}else{
			header("Location: bankexeat.php?status=error&msg=Fill All Fields");
		}
	}
					...
					<div class="row">
								...
								<div class="col-md-5 col-md-offset-1">
									<form name="form" id="form">
									  <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)" class="form-control">
										...
										<option value="bankexeat.php" selected>Bank Exeat</option>
									  </select>
									</form>
								</div>
							</div>
						...
						<form method="post" action="">
							...
							<div class="row form-control">
								<div class="form-group">
									...
									<div  class="col-md-6 col-md-offset-2">
										<input type="text" disabled value="Bank Exeat" class="form-control" name="leave-type" />
									</div>
								</div>
							</div>
							...
							<div class="row form-control">
								<div class="form-group">
									<div class="col-md-3">
										<label for="leave-type" class="form-group">Time of Leave:</label>
									</div>
									<div class="col-md-5 col-md-offset-1">
										<input  class="form-control" type="date" name="sleave"/>
									</div>
								</div>
							</div>
							...
							<div class="row form-control">
								<div class="form-group">
									<div class="col-md-3">
										<label for="leave-type" class="form-group">Time of Return:</label>
									</div>
									<div class="col-md-5 col-md-offset-1">
										<input class="form-control" type="date" name="sreturn"/>
									</div>
								</div>
							</div>
